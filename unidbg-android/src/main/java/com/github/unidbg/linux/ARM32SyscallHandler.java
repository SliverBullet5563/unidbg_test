...
public class ARM32SyscallHandler extends AndroidSyscallHandler {
    ...
    @Override
    public synchronized void hook(Backend backend, int intno, int swi, Object user) {
...
//                    emulator.attach().debug();
                ...
                case 240:
                    if (Translate.b)
                        emulator.attach().debug();
                    backend.reg_write(ArmConst.UC_ARM_REG_R0, futex2(emulator));
                    //切换线程时机
                    if (check_thread)
                        emulator.getThreadDispatcher().ExchangeThreadImmediately("futex 切换线程 threadId="+emulator.getThreadDispatcher().current_thread_id);
                    return;
                case 248:
                    exit_group(emulator);
                    return;
                ...
                case 150:
                    //mlock
                    backend.reg_write(ArmConst.UC_ARM_REG_R0, mlock(backend,emulator));
                    return;
            }
    ...
    private int mlock(Backend backend, Emulator<AndroidFileIO> emulator) {
        UnidbgPointer addr = UnidbgPointer.register(emulator, ArmConst.UC_ARM_REG_R0);
        int len = backend.reg_read(ArmConst.UC_ARM_REG_R1).intValue();
...
//        emulator.attach().debug();
    ...
    private int pthread_clone(Backend backend, Emulator<?> emulator) {
        ...
        // mov     ip, sp
        // stmfd   sp!, {r4, r5, r6, r7}       # save registers to parent stack
        // ldmfd   ip, {r4, r5, r6}            # load extra parameters
        // stmdb   r1!, {r5, r6}               # Push 'fn' and 'arg' onto the child stack.
        // ldr     r7, =__NR_clone             # Make the system call.
        ...
        if(ThreadDispatcher.lunch){
            emulator.getThreadDispatcher().hook_thread(emulator,backend);
            System.out.println("----开启多线程-------");

            threadId = ++ThreadDispatcher.thread_count_index;

            log.debug("pthread_clone child_stack=" + child_stack + ", thread_id=" + threadId + ", fn=" + fn.getPointer(48) + ", arg=" + arg + ", flags=" + list);

            emulator.getThreadDispatcher().threadMap.put(threadId, new LinuxThread(emulator,child_stack, fn, arg));

        }else {
            threadId = ++this.threadId;
            log.debug("pthread_clone child_stack=" + child_stack + ", thread_id=" + threadId + ", fn=" + fn + ", arg=" + arg + ", flags=" + list);
//            threadMap.put(threadId,new LinuxThread(emulator,child_stack, fn, arg));
            lastThread = threadId;
        }
        log.debug("pthread_clone threadId: "+threadId);
        return threadId;
    }
...
//                Inspector.inspect(data, "writev fd=" + fd + ", iov=" + iov + ", iov_base=" + iov_base);
    ...
    private static final int MUTEX_TYPE_BITS_NORMAL = 0x0;
    private static final int MUTEX_STATE_BITS_UNLOCKED = 0x0;
    private static final int MUTEX_STATE_BITS_LOCKED_UNCONTENDED = 0x1;
    ...
    private boolean check_thread;

    private int futex2(Emulator<?> emulator) {
        RegisterContext context = emulator.getContext();
        Pointer uaddr = context.getPointerArg(0);
        int futex_op = context.getIntArg(1);
        int val = context.getIntArg(2);

        long uaddr_peer = context.getLongArg(0);

        int old = uaddr.getInt(0);
        if (log.isWarnEnabled()) {
            log.warn(" futex uaddr=" + uaddr + ", _futexop=" + futex_op + ", op=" + (futex_op & 0x7f) + ", val=" + val + ", old=" + old+" waitThread="+emulator.getThreadDispatcher().waitThread);
        }

        ThreadDispatcher threadDispatcher = emulator.getThreadDispatcher();
        int threadId = threadDispatcher.current_thread_id;
        check_thread = false;
        ...
        switch (futex_op & 0x7f) {
            case FUTEX_WAIT:
                ...
                //关联futex uaddr和thread id
                LinuxFutex.getInstance().futex_q.put(new FutexWrapper(uaddr,uaddr_peer,futex_op,val,System.currentTimeMillis())
                                                                            ,threadId);
                //添加到阻塞线程
                threadDispatcher.waitThread.add(threadId);

                // TODO 不能在系统调用里切换线程，系统调用返回后，会修改R0寄存器的值
                ...
                int shared = val & 0x2000;
                if (log.isWarnEnabled()) {
                    log.warn("threadId="+threadId+" futex FUTEX_WAIT mytype=" + mytype + ", shared=" + shared + ", timeout=" + timeout + ", test=" + (mytype | shared)+" waitThread="+emulator.getThreadDispatcher().waitThread);
                }
//                uaddr.setInt(0, mytype | shared);
                return 0;
            case FUTEX_WAKE:
                if (val > 1){
                    emulator.getThreadDispatcher().waitThread.clear();
                    LinuxFutex.getInstance().futex_q.clear();
                    return 0;
                }
                //清除阻塞线程
                HashMap<FutexWrapper, Integer> futex_q = LinuxFutex.getInstance().futex_q;
                int thread_id = -1;
                for (FutexWrapper futexWrapper : futex_q.keySet()) {
                    //uaddr相同，唤醒阻塞在uaddr上的一个线程
                    if (futexWrapper.uaddr_peer == uaddr_peer){
                        thread_id = futex_q.get(futexWrapper);
                        emulator.getThreadDispatcher().waitThread.remove(thread_id);
                        futex_q.remove(futexWrapper);
                        if (ThreadDispatcher.lunch)
                            check_thread = true;
                        break;
                    }
                ...
                if (log.isWarnEnabled()) {
                    log.warn("futex FUTEX_WAKE thread_id="+thread_id+" waitThread="+emulator.getThreadDispatcher().waitThread);
                }
                return 0;
            default:
                throw new AbstractMethodError();
